...
@Component({
  selector: 'app-root',
  standalone: true,
  imports: [RouterOutlet, UploaderModule, DropDownListModule],
...
export class AppComponent {
  ...
  public onBeforefailure(args: any): void {
    ...
    let clearTimeInterval: any = setInterval(() => {
      if (
        navigator.onLine &&
        !isNullOrUndefined(proxy.chunkupload.filesData[0]) &&
        proxy.chunkupload.filesData[0].statusCode == 4
      ) {
        ...
        if (
          !proxy.isInteraction &&
          !isNullOrUndefined(proxy.chunkupload.filesData[0]) &&
          proxy.chunkupload.filesData[0].statusCode == 3
        ) {
